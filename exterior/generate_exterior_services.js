...
// 1. Generate Individual Service Pages
// We will use one of the existing inner pages as a template
...
if (fs.existsSync(templatePath)) {
    ...
    services.forEach(srv => {
        ...
        // Replace main background image
        newContent = newContent.replace(/background-image:\s*url\('[^']+'\)/g, `background-image: url('https://images.unsplash.com/photo-${srv.img}?auto=format&fit=crop&w=2000&q=80')`);
        
        fs.writeFileSync(path.join(exteriorDir, `${srv.slug}.html`), newContent);
    });
    console.log('Successfully generated 14 individual service pages.');
}

// 2. Update services.html grid
...
if (fs.existsSync(servicesHtmlPath)) {
    ...
    services.forEach((srv, i) => {
        ...
        <a href="${srv.slug}.html" class="group block" data-aos="fade-up" data-aos-delay="${delay}">
            <div class="relative h-[450px] rounded-[2rem] overflow-hidden mb-6">
    ...
    const gridStart = content.indexOf('<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">');
    if (gridStart !== -1) {
        const gridEnd = content.indexOf('</div>\n    </section>\n\n    <!-- 4. Our Process -->', gridStart);
        if (gridEnd !== -1) {
            content = content.substring(0, gridStart + '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">'.length) + '\n' + gridHtml + '\n        ' + content.substring(gridEnd);
            fs.writeFileSync(servicesHtmlPath, content);
            console.log('Successfully updated services.html grid.');
        }
    }
}

// 3. Update index.html swiper
...
if (fs.existsSync(indexHtmlPath)) {
    ...
    const wrapperStart = content.indexOf('<div class="swiper-wrapper" id="services-wrapper">');
    if (wrapperStart !== -1) {
        const wrapperEnd = content.indexOf('</div>\n        </div>\n\n        <!-- Custom Navigation -->', wrapperStart);
        if (wrapperEnd !== -1) {
            content = content.substring(0, wrapperStart + '<div class="swiper-wrapper" id="services-wrapper">'.length) + '\n' + swiperHtml + '          ' + content.substring(wrapperEnd);
            fs.writeFileSync(indexHtmlPath, content);
            console.log('Successfully updated index.html services swiper.');
        }
    } else {
        // Fallback for looking for generic swiper-wrapper inside the Services section
        const sectionStart = content.indexOf('<!-- 3. Services Section -->');
        if (sectionStart !== -1) {
            ...
            if (wrapStart !== -1 && wrapEnd !== -1) {
                ...
                fs.writeFileSync(indexHtmlPath, content);
                console.log('Successfully updated index.html services swiper (fallback).');
            }
        }
    }
}
